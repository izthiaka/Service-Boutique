...
export default class MatriculeGenerate {
  ...
  generateScoutMatricule(sexe: string, dateNaissance: Date, numero: number): string {
    const codeSexe = sexe.toLowerCase() === 'masculin' ? '1' : sexe.toLowerCase() === 'féminin' ? '2' : '';
  
    const annee = new Date(dateNaissance).getFullYear();
    const mois = (new Date(dateNaissance).getMonth() + 1).toString().padStart(2, '0');
    const jour = new Date(dateNaissance).getDate().toString().padStart(2, '0');
    const dateNaissanceFormattee = `${annee}${mois}${jour}`;
  
    const nombreAleatoire = Math.floor(Math.random() * 1000000).toString().padStart(6, '0');
